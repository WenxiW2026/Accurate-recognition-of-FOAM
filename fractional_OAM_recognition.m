...
%% polar expansion
img = imread('D:\1.5.png'); 
img = im2double(img);
img = img./max(max(img));
[row,col] = size(img);
% imshow(img);

[center, radius] = findDiscreteRingCenterAndRadius(img);
disp(['Symmetric center: (', num2str(center(1)), ', ', num2str(center(2)), ')']);
disp(['Symmetric radius: ', num2str(radius)]);


[smoothedProfile,min_valley_position,min_valley_value] = polarexpansion(img, center, radius);
...
%%
function [smoothedProfile,min_valley_position,min_valley_value] = polarexpansion(image, center, radius)
...
function [center, radius] = findDiscreteRingCenterAndRadius(image)
